...
func mcpBridgeFixture(blockers ...*int32) (a, b *SdkMcpServer) {
...
func TestClient_McpMessageDispatch(t *testing.T) {
	...
	if nfErr["code"] != float64(-32601) || !strings.Contains(nfErr["message"].(string), "nope-server") {
	...
	if !strings.Contains(phBlk["text"].(string), "panicked") {
	...
	if umErr["code"] != float64(-32601) || !strings.Contains(umErr["message"].(string), "bogus/method") {
